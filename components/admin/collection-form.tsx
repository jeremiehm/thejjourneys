...
export function CollectionForm({ collection, articles, breadcrumbs }: CollectionFormProps) {
  ...
  function handleSubmit() {
    sessionStorage.setItem(
      "admin-toast",
      collection?.id ? "Collection updated" : "Collection saved",
    );
  }
  ...
  return (
    <form action={saveCollection} onSubmit={handleSubmit} className="pb-12">
      ...
      <div className="grid gap-5 rounded-xl border border-stone-200 bg-white p-6 shadow-sm md:grid-cols-2">
        <AdminField label="Title" name="title" required defaultValue={collection?.title ?? ""} />
        <AdminField label="Slug" name="slug" defaultValue={collection?.slug ?? ""} placeholder="auto if empty" />
        <AdminTextarea
          label="Description"
          name="description"
        ...
        />
        <ImageUploadField name="cover_image_url" label="Cover image" defaultValue={collection?.cover_image_url} />
      ...
      <section className="mt-8 space-y-4 rounded-xl border border-stone-200 bg-white p-6 shadow-sm">
        <div>
          <h2 className="text-lg font-semibold text-stone-950">Collection layout</h2>
          <p className="mt-1 text-sm text-stone-500">
            Reorderable blocks, saved in <code className="text-xs">collections.layout</code>.
          </p>
        </div>
